...
def series_to_supervised(data, n_in=1, n_out=1, dropnan=True):
    ...
    df = pd.DataFrame(data).apply(lambda x: x.fillna(-9999), axis=0)  # Fill NaN with an impossible value (-9999)
...
def prepare_data(series, n_test, n_in, n_out):
    ...
    train, test = supervised_data.iloc[:-n_test, :], supervised_data.iloc[-n_test:, :]
    return train, test