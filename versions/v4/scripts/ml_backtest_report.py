...
<div class="header">
  <h1>{market_emoji} XGB+MMoE 2026 回测报告</h1>
  <p>🤖 Model: XGBoost Leaf + MMoE (3-task) | 📅 {today}</p>
...
<div class="footer">
  <p>📊 Coral Creek 量化平台 | XGB_leaf+MMoE Walk-Forward Backtest</p>
...
def send_email_report(html: str, market: str, chart_path: str = None):
    ...
    subject = f"📊 {market_name} XGB+MMoE 2026 回测报告 — {today}"
...
def send_push_notification(market: str, data: dict):
    ...
    # Build message
    lines = [
        f"📊 *{market_emoji} {market_name} XGB+MMoE 2026 回测*",
        "",
        f"💰 总收益: *{total_ret:+.1f}%*",
        f"📈 Sharpe: *{sharpe:.2f}*",
        f"🎯 胜率: *{win_rate:.0f}%*",
    ...
    results = nm.send_all(f"{market_name} XGB+MMoE 回测报告", msg)
...
def generate_and_send(market: str, send_email_flag: bool = True, send_push_flag: bool = True):
    ...
    import json
    print(f"\n{'='*60}")
    print(f"📊 {market} XGB+MMoE 2026 回测报告")
    print(f"{'='*60}")
