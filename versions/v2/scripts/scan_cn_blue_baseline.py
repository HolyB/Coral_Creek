...
用法示例:
    ...
    # 只看日线BLUE+黑马信号同时出现
    python scan_cn_signals_blue_only.py --period daily --with-heima
...
def process_single_stock(stock, thresholds=None, period_type=PeriodType.ALL, 
...
def _scan_batch(batch, max_workers=5, max_wait_time=300, thresholds=None, 
                period_type=PeriodType.ALL, with_heima=False, min_turnover=100):
    """扫描一批股票"""
    results = []
    problem_stocks = []
    completed_count = 0
    ...
    with tqdm(total=len(batch), desc="批次扫描进度") as pbar:
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = {
                executor.submit(process_single_stock, stock, thresholds, period_type, with_heima, min_turnover): stock 
...
def scan_in_batches(batch_size=500, cooldown=30, max_workers=5, start_batch=1, end_batch=None, 
    ...
    for batch_num in range(start_batch, end_batch + 1):
        ...
        results_df = _scan_batch(
            ...
            thresholds=thresholds, period_type=period_type,
        ...
        if all_results:
            ...
            interim_results.to_csv(f'cn_signals_blue_interim_{batch_num}.csv', index=False, encoding='utf-8-sig')
...
def main(batch_size=500, max_workers=30, start_batch=1, end_batch=None, thresholds=None, 
         send_email=True, period_type=PeriodType.ALL, with_heima=False, min_turnover=100):
    """主函数 - 仅扫描BLUE信号"""
    default_thresholds = {
        'day_blue': 100,
        'week_blue': 100,
    ...
    if not results.empty:
        ...
        filename = f'cn_signals_blue_{period_suffix}{heima_suffix}_{timestamp}.csv'
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='A股股票信号扫描器 - 仅扫描BLUE信号（多头）',
