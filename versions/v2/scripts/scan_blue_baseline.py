...
用法示例:
    ...
    # 只看日线BLUE+黑马信号同时出现
    python scan_signals_blue_only.py --period daily --with-heima
...
class SignalNotifier:
    ...
    @staticmethod
    def send_summary_email(results, signal_counts, period_type=PeriodType.ALL, only_dual_signals=False):
        """发送信号总结邮件"""
        if results.empty:
        ...
        if only_dual_signals:
            ...
            report_title = f"日周同时出现BLUE信号 ({period_desc})"
        else:
            report_stocks = results
            report_title = f"股票BLUE信号扫描 ({period_desc})"
...
def process_single_stock(symbol, thresholds=None, period_type=PeriodType.ALL, with_heima=False):
    """
    处理单个股票，仅扫描BLUE信号
...
def scan_signals_parallel(max_workers=30, batch_size=100, cooldown=5, limit=None, thresholds=None, 
                          period_type=PeriodType.ALL, send_email=True, with_heima=False):
    """并行扫描股票信号"""
    tickers = get_all_tickers()
    ...
    with tqdm(total=batch_count, desc="Batch Progress") as batch_pbar:
        for i in range(0, len(tickers), batch_size):
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                futures = {executor.submit(process_single_stock, symbol, thresholds, period_type, with_heima): symbol for symbol in batch_tickers}
...
def main(limit=None, thresholds=None, send_email=True, only_dual_signals=False, 
         period_type=PeriodType.ALL, with_heima=False):
    """
    主函数 - 仅扫描BLUE信号
    ...
    print("\n" + "=" * 60)
    print(f"扫描模式: BLUE信号(多头){heima_desc}")
    ...
    results = scan_signals_parallel(
        ...
        thresholds=default_thresholds, period_type=period_type, 
        send_email=False, with_heima=with_heima
    )
    
    if not results.empty:
        results['company_name'] = results['symbol'].map(lambda x: COMPANY_INFO.get(x, 'N/A'))
        
        print(f"\n发现信号的股票 (BLUE信号{heima_desc}, {period_desc})：")
        print("=" * 180)
        ...
        if period_type == PeriodType.ALL:
            ...
            if not dual_signal_stocks.empty:
                ...
                print("=" * 140)
                print(f"共发现 {len(dual_signal_stocks)} 只股票日线和周线同时出现BLUE信号")
        
        # 保存结果到CSV
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        period_suffix = period_type if period_type != PeriodType.ALL else "all"
        csv_filename = f"signals_blue_{period_suffix}_{timestamp}.csv"
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='股票信号扫描器 - 仅扫描BLUE信号（多头）',
