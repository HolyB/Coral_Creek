...
class SignalNotifier:
    ...
    def send_signal_email(self):
        subject = f"港股股票信号通知: {self.symbol} 出现交易信号"
        
        body = f"股票代码: {self.symbol}\n"
        if 'name' in self.data:
            body += f"公司名称: {self.data['name']}\n"
        body += f"当前价格: {self.data['price']:.2f} HKD\n"
        body += f"成交额(万港币): {self.data['turnover']:.2f}\n\n"
        ...
        try:
            ...
            server.quit()
            print(f"港股 {self.symbol} 的信号通知邮件发送成功")
            return True
        except Exception as e:
            print(f"港股 {self.symbol} 的信号通知邮件发送失败: {e}")
    ...
    def send_batch_signal_email(stocks_data):
        ...
        subject = f"港股股票信号批量通知: 检测到 {len(stocks_data)} 只港股信号"
        
        body = f"检测到 {len(stocks_data)} 只港股出现交易信号:\n\n"
        
        for i, stock in enumerate(stocks_data, 1):
            body += f"{i}. 股票代码: {stock['symbol']}\n"
            if 'name' in stock:
                body += f"   公司名称: {stock['name']}\n"
            body += f"   价格: {stock['price']:.2f} HKD, 成交额(万港币): {stock['turnover']:.2f}\n"
        ...
        body += "-" * 40 + "\n"
        body += f"共检测到 {len(stocks_data)} 只港股有信号\n"
        ...
        try:
            print(f"正在尝试发送批量港股信号通知邮件...")
            server = smtplib.SMTP('smtp.gmail.com', 587)
            server.starttls()
            server.login(sender_email, email_password)
            server.sendmail(sender_email, receiver_emails, msg.as_string())
            server.quit()
            print(f"批量港股信号通知邮件发送成功，包含 {len(stocks_data)} 只港股")
            return True
        except Exception as e:
            print(f"批量港股信号通知邮件发送失败: {e}")
            print(f"详细错误信息: {traceback.format_exc()}")
            return False
    ...
    def send_summary_email(results, signal_counts):
        ...
        subject = f"港股日周信号扫描总结 ({current_time})"
        
        body = f"港股日周信号扫描总结 ({current_time})\n\n"
        ...
        for signal, count in signal_counts.items():
            if count > 0:
                body += f"{signal:<15}: {count:>5} 只\n"
        body += "-" * 40 + "\n"
        body += f"共发现 {len(results)} 只港股有信号\n\n"
        
        body += "港股列表：\n"
        body += "=" * 100 + "\n"
        body += f"{'代码':<8} | {'公司名称':<20} | {'价格(HKD)':>10} | {'成交额(万港币)':>15} | {'信号':<40}\n"
        body += "-" * 100 + "\n"
        ...
        for _, row in results.iterrows():
            ...
            body += f"{row['symbol']:<8} | {company_name:<20} | {row['price']:10.2f} | {row['turnover']:15.2f} | {signals_str:<40}\n"
        ...
        try:
            print(f"正在尝试发送港股日周信号总结邮件...")
            server = smtplib.SMTP('smtp.gmail.com', 587)
            server.starttls()
            server.login(sender_email, email_password)
            server.sendmail(sender_email, receiver_emails, msg.as_string())
            server.quit()
            print(f"邮件发送成功，包含 {len(results)} 只港股")
            return True
        except Exception as e:
            print(f"邮件发送失败: {e}")
            print(f"详细错误信息: {traceback.format_exc()}")
...
def get_hk_tickers():
    ...
    try:
        # 获取港股股票基本信息
        stock_info = pro.hk_basic(list_status='L', fields='ts_code,symbol,name,fullname,enname,market')
        
        if stock_info is None or stock_info.empty:
            logging.error("从tushare获取港股列表失败")
            return pd.DataFrame()
        
        # 过滤港股（排除指数、债券等）
        stock_info = stock_info[stock_info['ts_code'].str.contains(r'\.(HK)$', regex=True)]
        
        # 重命名列以保持与原代码一致
        tickers = []
        for _, row in stock_info.iterrows():
            tickers.append({
                'code': row['ts_code'],  # tushare格式：00700.HK
                'name': row['name'] if row['name'] else row.get('enname', 'N/A')
            })
        
        logging.info(f"从tushare获取到 {len(tickers)} 只港股")
        return pd.DataFrame(tickers)
        
    except Exception as e:
        logging.error(f"获取港股列表失败: {e}")
        return pd.DataFrame()
...
def get_hk_stock_data_tushare(ts_code, period='D', start_date=None, end_date=None):
    """使用tushare获取港股历史数据"""
    max_retries = 3
    retry_count = 0
    ...
    while retry_count < max_retries:
        try:
            # 添加小延迟避免API频率限制
            time.sleep(0.8)  # 增加延迟时间避免港股API频率限制
            ...
            if "每分钟最多访问" in error_msg or "访问频率" in error_msg:
                wait_time = min(60 * retry_count, 180)  # 递增等待时间，最多3分钟
                logging.warning(f"API频率限制 {ts_code}，等待 {wait_time} 秒后重试 (重试 {retry_count}/{max_retries})")
                time.sleep(wait_time)
            else:
                logging.error(f"获取港股数据失败 {ts_code}: {e}")
                if retry_count < max_retries:
                    time.sleep(1 * retry_count)  # 其他错误短暂等待
                else:
                    break
    
    logging.error(f"获取港股数据最终失败 {ts_code}，已重试 {max_retries} 次")
    return None

def process_single_hk_stock(stock, thresholds=None, require_both_timeframes=True, signal_type='both', min_turnover=100):
    """处理单个港股，支持配置日周信号是否需同时满足及信号类型
    Args:
        ...
        min_turnover: 最小成交额（万港币），默认100万
    """
    default_thresholds = {
        'day_blue': 150,
        'day_lired': -150,
    ...
    symbol = stock['code']  # tushare格式：00700.HK
    name = stock['name']
    
    try:
        logging.info(f"开始处理港股: {symbol} {name}")
        
        # 使用tushare获取港股日线数据
        data_daily = get_hk_stock_data_tushare(symbol, period='D')
        if data_daily is None or data_daily.empty:
            with print_lock:
                logging.warning(f"获取港股日线数据出错 ({symbol} {name}): 数据为空")
            return None
        ...
        if signal_detected:
            turnover = latest_daily['Volume'] * latest_daily['Close'] / 10000  # 转换为万港币
            
            # 添加成交额过滤条件
            if turnover < min_turnover:
                logging.info(f"{symbol} 成交额{turnover:.2f}万港币 < {min_turnover}万港币，忽略信号")
                return None
            ...
            logging.info(f"{symbol} 发现信号，成交额: {turnover:.2f}万港币")
            return result
        ...
        with print_lock:
            logging.error(f"获取港股数据出错 ({symbol} {name}): {str(e)}")
            traceback.print_exc()
        return None

def _scan_hk_batch(batch, max_workers=5, max_wait_time=300, thresholds=None, require_both_timeframes=True, signal_type='both', min_turnover=100):
    results = []
    problem_stocks = []
    completed_count = 0
    ...
    with tqdm(total=len(batch), desc="港股批次扫描进度") as pbar:
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = {executor.submit(process_single_hk_stock, stock, thresholds, require_both_timeframes, signal_type, min_turnover): stock for _, stock in batch.iterrows()}
            ...
            while completed_count < len(batch):
                if time.time() - start_time > max_wait_time:
                    with print_lock:
                        ...
                        logging.warning(f"已等待 {max_wait_time} 秒，仍有 {remaining} 只港股未完成。继续处理下一批...")
    ...
    if problem_stocks:
        ...
        problem_df.to_csv(f'problem_hk_stocks_{timestamp}.csv', index=False, encoding='utf-8-sig')
        with print_lock:
            logging.info(f"本批次有 {len(problem_stocks)} 只问题港股，已保存到 problem_hk_stocks_{timestamp}.csv")
    
    return pd.DataFrame(results) if results else pd.DataFrame()

def scan_hk_in_batches(batch_size=300, cooldown=60, max_workers=10, start_batch=1, end_batch=None, max_wait_time=300, thresholds=None, send_email=True, require_both_timeframes=True, signal_type='both', min_turnover=100):
    logging.info("正在获取港股列表...")
    stock_list = get_hk_tickers()
    ...
    logging.info(f"共获取到 {total_stocks} 只港股")
    ...
    for batch_num in range(start_batch, end_batch + 1):
        ...
        logging.info(f"开始扫描第 {batch_num}/{batch_count} 批次 ({len(batch)} 只港股)...")
        
        batch_start_time = time.time()
        results_df = _scan_hk_batch(batch, max_workers=max_workers, max_wait_time=max_wait_time, thresholds=thresholds, require_both_timeframes=require_both_timeframes, signal_type=signal_type, min_turnover=min_turnover)
        batch_end_time = time.time()
        
        if not results_df.empty:
            all_results.append(results_df)
            logging.info(f"批次 {batch_num} 发现 {len(results_df)} 只有信号的港股")
        else:
            logging.info(f"批次 {batch_num} 未发现港股信号")
        ...
        if all_results:
            ...
            interim_results.to_csv(f'hk_signals_interim_{batch_num}.csv', index=False, encoding='utf-8-sig')
            logging.info(f"已将港股中间结果保存到 hk_signals_interim_{batch_num}.csv")
    ...
    if all_results:
        ...
        if send_email and not final_results.empty:
            try:
                ...
                logging.error(f"发送港股信号总结邮件失败: {e}")
...
def main(batch_size=300, max_workers=5, start_batch=1, end_batch=None, thresholds=None, send_email=True, require_both_timeframes=True, signal_type='both', min_turnover=100):
    default_thresholds = {
        'day_blue': 100,
        'day_lired': -100,
        'week_blue': 100,
    ...
    logging.info(f"最小成交额要求: {min_turnover}万港币")
    ...
    if not stock_list.empty:
        ...
        print(f"总计港股数量: {len(stock_list)}")
    
    # 扫描港股
    results = scan_hk_in_batches(
        batch_size=batch_size,
        cooldown=120,  # 增加批次间冷却时间到2分钟
        max_workers=3,  # 使用更少的并发线程
    ...
    if not results.empty:
        print("\n发现信号的港股：")
        print("=" * 170)
        print(f"{'代码':<12} | {'公司名称':<25} | {'价格(HKD)':>10} | {'成交额(万港币)':>15} | "
        ...
        for _, row in results.iterrows():
            ...
            print(f"{row['symbol']:<12} | {company_name:<25} | {row['price']:10.2f} | {row['turnover']:15.2f} | "
        ...
        print("-" * 40)
        print(f"共发现 {len(results)} 只港股有信号")
        
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f'hk_signals_tushare_{timestamp}.csv'
    ...
    end_time = time.time()
    print(f"\n港股扫描完成，耗时: {end_time - start_time:.2f} 秒")

if __name__ == "__main__":
    # 示例：只看 BLUE 信号，日周单独满足，成交额大于100万港币
    main(
        batch_size=200,  # 减小批次大小
        max_workers=3,   # 减少并发线程数
        start_batch=1,
        end_batch=None,
        thresholds=None,
        send_email=True,
        ...
        signal_type='blue',
        min_turnover=100  # 设置最小成交额为100万港币
    ) 